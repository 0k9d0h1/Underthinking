...
def main():
    ...
    ap.add_argument(
        "--src", required=True, help="HF repo or local path with the original weights"
    )
    ap.add_argument(
        "--dst",
        required=True,
    ...
    ap.add_argument("--target_layer", type=int, default=27)
    ...
    custom = Qwen2ForCausalLMCustom(cfg).to("cpu")
