...
def main(args):
    ...
    input_files = glob.glob(f'./{model_name.split('/')[-1].replace('-', '_')}/*_generation.jsonl')
    evaluation_files = glob.glob(f'./{model_name.split('/')[-1].replace('-', '_')}/*_evaluation.jsonl')
    output_file = f"./{model_name.split('/')[-1].replace('-', '_')}/thought_clustering.jsonl"
    split_output_file = f"./{model_name.split('/')[-1].replace('-', '_')}/thought_split.jsonl"
