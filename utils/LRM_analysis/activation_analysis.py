...
def load_data(split_path, cluster_path):
    with open(split_path)   as f1, \
         open(cluster_path) as f2:
...
def cluster_metrics(true_seq, reps_layer, seed):
    ...
    if not uniq:
        return None
    kmeans = KMeans(len(uniq), n_init="auto", random_state=seed)\
...
def detect_burst_heads_2d(
    ...
    for h in range(H):
        ...
        def torch_maximum_filter_2d(A, size=3):
            ...
            return pool[0, 0]  # (S, S)
        local_max = (A == torch_maximum_filter_2d(A.to("cuda:1"), size=nms_size)).to("cpu")
...
def main(args):
    ...
    cluster_path = f"./{model_name.split('/')[-1].replace('-', '_')}/thought_clustering.jsonl"
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "per_layer_clustering.png"); plt.close()
    ...
    for i, (within_score, inter_score) in enumerate(zip(within_scores, inter_scores)):
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f"within_vs_inter_attention_{i}.png"); plt.close()
    ...
    for i, (within_score, inter_score) in enumerate(zip(within_scores, inter_scores)):
        ...
        plt.ylabel("#Problems")
        plt.tight_layout()
        plt.savefig(output_dir / f"attention_ratio_hist_{i}.png"); plt.close()
    ...
    print(f"All figures & data stored in: {output_dir.resolve()}")
...
help="Name of the model to use for analysis.")
    ...
    parser.add_argument("--batch_size_limit", type=int, default=24000,
                        help="Maximum number of tokens per forward pass.")
    parser.add_argument("--output_dir", type=str, default="LRM_analysis_output",
                        help="Directory to save output files.")
    parser.add_argument("--spike_th", type=float, default=100.0,
                        help="Threshold for spike detection.")
    parser.add_argument("--burst_z", type=float, default=5.0,
                        help="Z-score threshold for burst detection.")
    parser.add_argument("--nms_size", type=int, default=5,
