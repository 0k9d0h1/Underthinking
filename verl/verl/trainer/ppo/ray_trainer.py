...
from verl.trainer.ppo.reward import compute_reward, compute_reward_async, compute_reward_ppl
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.PPL:
        advantages, returns = core_algos.compute_ppl_outcome_advantage(
            reward_for_phrases=data.non_tensor_batch["reward_for_phrases"],
            phrase_token_lengths=data.non_tensor_batch["phrase_token_lengths"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
        )
        data.batch["advantages"] = advantages
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.PPL:
                            batch.non_tensor_batch["phrase_token_lengths"] = phrase_token_lengths
                            batch.non_tensor_batch["reward_for_phrases"] = reward_for_phrases
                        else:
