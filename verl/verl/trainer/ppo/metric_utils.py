...
def compute_data_metrics_ppl(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    """
    Computes various metrics from a batch of data for PPO training.
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - prompt_length/mean, max, min, clip_ratio: Statistics about prompt lengths
    """
    sequence_score = torch.Tensor([a[a != float("-inf")].mean(-1) for a in batch.non_tensor_batch["reward_for_phrases"]])
    sequence_reward = sequence_score.clone()
