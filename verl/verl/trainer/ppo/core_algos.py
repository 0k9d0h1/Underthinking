...
def compute_ppl_outcome_advantage(
    reward_for_phrases,
    ...
    with torch.no_grad():
        bsz = len(reward_for_phrases)
        for i in range(bsz):
            id2reward_for_phrases[index[i]].extend(reward_for_phrases[i])
        for idx in id2reward_for_phrases:
            if len(id2reward_for_phrases[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
            elif len(id2reward_for_phrases[idx]) > 1:
                id2mean[idx] = torch.mean(torch.tensor(id2reward_for_phrases[idx]))
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                ...
                reward_for_phrases[i] = reward_for_phrases[i] - id2mean[index[i]]
        for i in range(response_mask.shape[0]):
            assert response_mask[i].sum() == sum(phrase_token_lengths[i]), f"response_mask {response_mask[i]} and phrase_token_lengths {phrase_token_lengths[i]} do not match"
            indices = (response_mask[i] == 1).nonzero(as_tuple=True)[0]
            start = indices[0].item()

            for j, length in enumerate(phrase_token_lengths[i]):
                scores[i, start:start + length] = reward_for_phrases[i][j]
                start += length
